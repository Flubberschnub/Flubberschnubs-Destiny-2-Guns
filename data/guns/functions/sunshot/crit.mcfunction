...
execute positioned ~ ~ ~ if entity @e[distance=..1,type=#minecraft:mobs,scores={hp=..24}] run scoreboard players set Kill shootgun 1
tag @e[distance=..1,type=#minecraft:mobs] add aestd.damage.hurt_by_player
tag @e[distance=..1,type=#aestd1:bosses] add aestd.damage.hurt_by_player
execute as @e[distance=..1,type=#minecraft:mobs] run scoreboard players set @s aestd.damage 2400
